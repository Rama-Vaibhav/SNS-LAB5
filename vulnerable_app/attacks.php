...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  switch ($attack) {
    ...
    case 'db_insert':
      ...
      $before_rows = [];
      while ($r = mysqli_fetch_assoc($before))
        $before_rows[] = $r;
      ...
      $after_rows = [];
      while ($r = mysqli_fetch_assoc($after))
        $after_rows[] = $r;
    ...
    <main class="main-panel">
      ...
      <?php if ($attack_name): ?>
        <div class="result-card">
          ...
          <?php if ($error_msg): ?>
            <div class="error-box">SQL Error (intentionally displayed — insecure
              practice):<br><?= htmlspecialchars($error_msg) ?></div>
