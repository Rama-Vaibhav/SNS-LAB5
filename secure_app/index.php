...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    h1 { font-size: 1.6rem; font-weight: 600; color: var(--text); margin-bottom: 4px; }
    .subtitle { font-size: 0.78rem; color: var(--dim); font-family: var(--mono); margin-bottom: 28px; }
    ...
    label {
      display:block; font-size:0.72rem; font-weight:500; letter-spacing:.07em;
    ...
    input[type="text"], input[type="password"] {
      ...
      font-size:0.9rem; padding:10px 14px; outline:none; transition:border-color .2s;
    }
    input:focus { border-color: var(--accent); background:#fff; }

    button[type="submit"] {
      width:100%; background:var(--accent); color:#fff; border:none;
      border-radius:6px; font-family:var(--sans); font-size:0.95rem;
      font-weight:600; padding:12px; cursor:pointer; margin-top:8px;
      transition:background .2s, transform .1s;
    }
    button[type="submit"]:hover  { background:var(--accent2); }
    button[type="submit"]:active { transform:scale(0.98); }

    .alert { border-radius:6px; padding:12px 16px; font-size:0.82rem;
             margin-bottom:18px; border:1px solid; }
    .alert-success { background:#f0fdf4; border-color:#86efac; color:var(--success); }
    .alert-error   { background:#fef2f2; border-color:#fca5a5; color:var(--error);   }

    .security-note {
      margin-top:22px; padding:14px;
      background:#eff6ff; border:1px solid #bfdbfe; border-radius:6px;
      font-family:var(--mono); font-size:0.72rem; color:#1d4ed8; line-height:1.8;
    }
    .security-note strong { display:block; margin-bottom:6px; font-size:.67rem;
